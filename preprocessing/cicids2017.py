...
def load():
    ...
    # reduce leakage (dest port kept on purpose)
    columns_to_drop = []
    ...
    # 70/30 split
    x_train_raw, x_test_raw, y_train, y_test = train_test_split(x, y, test_size=0.30, stratify=y, random_state=RANDOM_STATE)
    ...
    print(f"VarianceThreshold: {x_train_raw.shape[1]} -> {x_train_variance.shape[1]} features")

    # definitely want to know what got dropped!
    if n_dropped:
        dropped_features = x_train_raw.columns[~variance_filter.get_support()].tolist()
        print(f"VarianceThreshold dropped {n_dropped}:\n{dropped_features}")

    # StandardScaler - same with variance
    ...
    print("Count of 'Benign' in y_train:", (y_train == 'BENIGN').sum())
